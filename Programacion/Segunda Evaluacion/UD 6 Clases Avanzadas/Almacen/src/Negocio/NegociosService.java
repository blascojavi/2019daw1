...
public class NegociosService {
    ...
    public void introducirVenta(int nc, int np, String nv) {
        try {

            Cliente clienteVenta = null;
            for (int i = 0; i < clientes.size() && clienteVenta == null; i++) {
                if (clientes.get(i).getIdCliente() == nc) {
    ...
    public Venta datosVenta(Cliente c, Producto p) {
        Venta v = new Venta();
        v.setCliente(c);
        v.setVendedor(c.getNombre());
        v.setProducto(p);
        v.setPrecioVenta();
    ...
    public void eliminarCliente(int n) {
        try {
            // Al eliminar un cliente también eliminamos
            //Las ventas asociadas a el
            ...
            for (int i = 0; i < clientes.size() && clientes.get(i).getIdCliente() != n; i++) {
